...
require "components/progress-bar.component.php";

if (is_admin()) {
	require "lib/class-civique-github-updater.php";
	new Civique_Github_Updater();
}
...
if (!function_exists("civique_after_setup_theme")):
	function civique_after_setup_theme()
	{
		global $wp_version;
		...
		$formats = ["gallery", "image", "aside", "link"];
		...
		$header_args = [
			"default-image" => get_template_directory_uri() . "/headers/1.jpg",
			"width" => 1280,
			"height" => 400,
			"flex-width" => false,
			"flex-height" => false,
		...
		$header_images = [
			"mist" => [
				"url" => get_template_directory_uri() . "/headers/1.jpg",
				"description" => "Mist",
			],
			"sea" => [
				"url" => get_template_directory_uri() . "/headers/2.jpg",
				"description" => "Sea",
			],
			"trees" => [
				"url" => get_template_directory_uri() . "/headers/3.jpg",
				"description" => "Trees",
			],
			"waterfall" => [
				"url" => get_template_directory_uri() . "/headers/4.jpg",
	...
	add_action("after_setup_theme", "civique_after_setup_theme");
endif;
...
if (!function_exists("civique_register_widgets")):
	...
	function civique_register_widgets()
	{
		...
		require "widget-fundraiser-bloomerang.php";
		// register_widget("Civique_Fundraiser_Widget");
		register_widget(Civique_Fundraiser_Widget::class);
		register_widget(Civique_Bloomerang_Campaign_Widget::class);
	}
...
function omega_non_profit_shortcode()
{
	...
	$url = sprintf(
		"https://projects.propublica.org/nonprofits/api/v1/organizations/%s.json",
		get_theme_mod("np_ein"),
	);
	...
	$rtn .= sprintf(
		...
		sprintf(
			"%s-%s",
			substr($json->organization->ein, 0, 2),
			substr($json->organization->ein, 2),
		),
	);
	$rtn .= sprintf(
		$dt,
		"Section",
		sprintf("501(c)(%s)", $json->organization->subsection_code),
	);
	$rtn .= sprintf(
		$dt,
		"Address",
	...
	if (!empty($json->filings_with_data)) {
		$rtn .= sprintf(
			"<h3>%s</h3><p>%s</p>",
			"Filings with Data",
			"The following data are extracted from IRS Form 990 filings.  Click the year to see the entire filing.",
		);
		$rtn .= "<table>";
		$rtn .=
			"<thead> <tr> <th>Tax Year</th> <th>Revenue</th> <th>Expenses</th> <th>Ending Assets</th> <th>Ending Liabilities</th> </tr> </thead>";
		foreach ($json->filings_with_data as $filing) {
			$rtn .= "<tr>";
			$rtn .= sprintf(
				'<td><a href="%s">%s (pdf)</a></td>',
				$filing->pdf_url,
				$filing->tax_prd_yr,
			);
	...
	if (!empty($json->filings_without_data)) {
		$rtn .= sprintf(
			...
			"The following filings have not had data extracted.  Click the year to see the entire filing.",
		);
		$rtn .= "<ul>";
	...
	$rtn .= sprintf(
		"<h4>Data Provided By</h4><p>%s</p>",
		make_clickable(nl2br($json->data_source)),
	);
...
if (!function_exists("omega_archive_pagination")):
	function omega_archive_pagination($pages = "", $range = 3)
	{
		...
		if (1 != $pages) {
			...
			echo sprintf(
				'<div class="row next-prev"><div class="col-md-6">%s</div><div class="col-md-6">%s</div></div>',
				$paged > 1
					? get_previous_posts_link("Previous Page")
					: "<em>This is the first page</em>",
				$paged < $pages
					? get_next_posts_link("Next Page")
					: "<em>This is the last page</em>",
			);
			echo sprintf(
				'<span class="meta">Page&nbsp;%u&nbsp;of&nbsp;%u</span> ',
				$paged,
				$pages,
			);
			...
			if ($paged > 2 && $paged > $range + 1 && $showitems < $pages) {
				echo sprintf(
					'<span class="inactive"><a href="%1$s">%2$s</a></span> ',
					get_pagenum_link(1),
					"&laquo;&nbsp;First",
				);
			}
			...
			for ($i = 1; $i <= $pages; $i++) {
				if (
					...
					if ($paged == $i) {
						...
						echo sprintf(
							'<span class="%1$s"><a href="%3$s">%2$u</a></span> ',
							"inactive",
							$i,
							get_pagenum_link($i),
						);
