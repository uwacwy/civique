...
function print_text_setting_row($label, $field_name, $description = null)
{
	...
	<tr valign="top">
		...
		<td>
			<input name="<?php echo esc_attr(
   	$field_name,
   ); ?>" type="text" id="<?php echo esc_attr($field_name); ?>" value="<?php echo esc_attr(get_option($field_name)); ?>" class="regular-text" />
			<?php if (
   	!empty($description)
   ) { ?><p class="description"><?php echo htmlspecialchars(
	$description,
); ?></p><?php } ?>
		</td>
	</tr>
...
function print_list_setting_row(
	$label,
	$field_name,
	$values,
	$description = null,
...
<div class="wrap">
	...
	<form method="post" action="options.php">
   	...
   	'The username for the location\'s Twitter Profile.  No @-sign.',
		...
		<textarea id="lhs_home_intro" class="large-text code" rows="7" name="lhs_home_intro"><?php echo htmlspecialchars(
  	get_option("lhs_home_intro"),
  ); ?></textarea>
		...
		<textarea id="lhs_custom_css" class="large-text code" rows="10" name="lhs_custom_css"><?php echo htmlspecialchars(
  	get_option("lhs_custom_css"),
  ); ?></textarea>
